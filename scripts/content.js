...
function extract() {
  ...
  if (coursesection) {
    ...
    for (var nodo of course_nodes) {
      ...
      courses.push(getCleanText(courseName));
    }
  }
  ...
  if (projectsection) {
    var project_nodes =
      projectsection.querySelectorAll("div > ul > li") || null;
    for (var nodo of project_nodes) {
      var projectName = nodo.textContent;
      projects.push(getCleanText(projectName));
    }
  }
  ...
  if (languagesection) {
    ...
    for (var nodo of lang_nodes) {
      ...
      languages.push(getCleanText(language));
    }
  }
  ...
  if (volnodes) {
    for (var nodo of volnodes) {
      ...
      var vol_company =
        nodo.querySelector("h4")?.textContent.replace("Company Name", "") ||
        null;
      var vol_location =
        nodo.querySelector(".pv-entity__location span:nth-child(2)")
          ?.textContent || null;
      var vol_description =
        nodo.querySelector(".pv-entity__extra-details")?.textContent || null;
      var date1 =
        nodo.querySelector(".pv-entity__date-range span:nth-child(2)")
          ?.textContent || null;
      var date2 =
        nodo.querySelector(".pv-entity__bullet-item")?.textContent || null;
        ...
        for (j = 0; j < elems.length; j++) {
          ...
          for (k = 0; k < kchilds.length; k++) {
            ...
            if (k == 0)
              //role name
              rname =
                kchilds[k]?.querySelector("span > span").textContent || "";
            if (k == 1) {
              //role duration
              var ta = kchilds[k]
                .querySelector("span")
